...
import { ExceptSequence } from '../../../../src/sequences/implementations/except-sequence';
import { ICat } from '../../../models/i-cat';
import { CatComparator } from '../../../utils/cat-comparator';

describe('ExceptSequence', () => {
    describe('Creation', () => {
        it('from other seq', () => {
            const temp = new Sequence<number>([1, 2, 3, 4]);
            const result = new ExceptSequence(temp, new Sequence([3, 4, 5]), new DefaultComparator());

            expect(result).not.equal(undefined);
            expect(result.toArray()).deep.equal([1, 2]);
        });
        ...
        it('should use comparator', () => {
            ...
            const result = new ExceptSequence(temp, new Sequence(second), new CatComparator());
